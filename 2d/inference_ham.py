...
def inference(args, model, db_config, test_save_path=None, no_of_attr=3, log_folder="./results.txt"):
    ...
    with open(log_folder + '/result_all.csv', 'w') as file:
        file.write("\nno,ID,Dice,IoU,Attr")

    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        
        image, label, case_name, attr_label = sampled_batch['image'], sampled_batch['label'], sampled_batch['pid'], sampled_batch['attr_label']
        ...
        with open(log_folder + '/result_all.csv', 'a') as file:
    ...
    for one_attr in range(no_of_attr):
        ...
        logging.info('\n----------- Lession %s - %d (n=%d) Dice IoU -----------' % (args.attribute, one_attr, len(one_attr_dice_list)))
        logging.info('%.3f & %.3f' % (np.mean(one_attr_dice_list), np.mean(one_attr_jc_list)) )
        logging.info('\n----------- Lession %s - %d (n=%d)  -----------' % (args.attribute, one_attr, len(one_attr_dice_list)))
