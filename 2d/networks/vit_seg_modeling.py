...
class SparseMoE(nn.Module):
    def __init__(self, n_embed, num_experts=8, top_k=2, max_attr=3):
        super(SparseMoE, self).__init__()
        self.router = nn.ModuleList([NoisyTopkRouter(n_embed, num_experts, top_k) for _ in range(max_attr)])
...
class Encoder(nn.Module):
    def __init__(self, config, vis):
        ...
        self.flag_dmoe = False
        if config.dmoe:
            self.flag_dmoe = True
            self.sparseMoE = SparseMoE(config.hidden_size, num_experts=8, top_k=2, max_attr = config['n_attr'])

    def forward(self, hidden_states, dmoe=None):
        attn_weights = []
        features = []
        for i, layer_block in enumerate(self.layer):
            ...
            # dMoE
            ####################################
            if (self.flag_dmoe) & (dmoe is not None):
                
                # Apply the FFN
                # sparseMoE_keys = self.sparseMoE[i](hidden_states, dmoe)
                sparseMoE_keys = self.sparseMoE(hidden_states, dmoe)

                # Add the residual connection (input to FFN) to the FFN output
                hidden_states = hidden_states + sparseMoE_keys
            ####################################
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids, dmoe=None):
        embedding_output, features = self.embeddings(input_ids)
        encoded, attn_weights, features = self.encoder(embedding_output, dmoe)  # (B, n_patch, hidden)
        return encoded, attn_weights, features
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        if self.config.n_skip != 0:
            ...
            for i in range(4-self.config.n_skip):  # re-select the skip channels according to n_skip
                skip_channels[3-i]=0
            skip_channels = [config.hidden_size, config.hidden_size, config.hidden_size, config.hidden_size]
    ...
    def forward(self, hidden_states, features=None, dmoe=None):
        ...
        for i, decoder_block in enumerate(self.blocks):
            if (features is not None) & (self.config.n_skip > 0):
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=5, zero_head=False, vis=False, dmoe=False):
    ...
    def forward(self, x, dmoe=None):
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        x, attn_weights, features = self.transformer(x, dmoe)  # (B, n_patch, hidden)
        x = self.decoder(x, features, dmoe)
